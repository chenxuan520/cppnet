...
TEST(SSLContext, SSLConnect) {
  ...
  Socket tmp_soc;
  rc = tmp_soc.Init();
  MUST_TRUE(rc == 0, tmp_soc.GetSysErr());
  tmp_soc.SetReuseAddr();

  auto recv_ssl_sock = ssl_ctx.CreateSSLSocket(tmp_soc);
  ...
  MUST_TRUE(recv_msg == "hello", "recv data error " + recv_soc->GetSysErr());
}
